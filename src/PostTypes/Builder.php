...
class Builder
{
    ...
    /**
     * Modify the column for the PostType
     * @param  array  $column  Default WordPress column
     * @return array            The modified column
     */
    public function modifyColumns($column)
    {
        return $this->posttype->column->modifyColumns($column);
    }
    /**
     * Populate custom column for the PostType
     * @param  string $column   The column slug
     * @param  int    $post_id  The post ID
     */
    public function populateColumns($column, $post_id)
    {
        if (isset($this->posttype->column->populate[$column])) {
            call_user_func_array($this->posttype->column()->populate[$column], [$column, $post_id]);
        }
    }

    /**
     * Make custom column sortable
     * @param array  $column  Default WordPress sortable column
     */
    public function setSortableColumns($column)
    {
        if (!empty($this->posttype->column()->sortable)) {
            $column = array_merge($column, $this->posttype->column()->sortable);
        }
    ...
    /**
     * Set query to sort custom column
    ...
    public function sortSortableColumns($query)
    {
        ...
        if ($this->posttype->column()->isSortable($orderby)) {
            // get the custom column options
            $meta = $this->posttype->column()->sortableMeta($orderby);
