...
class RenameCommand extends Base implements CommandInterface {
	public function handle( array $args = array() ) {
			...
			if ( false !== $raw ) {
				...
				if ( $newNs && isset( $data['autoload']['psr-4'] ) && is_array( $data['autoload']['psr-4'] ) ) {
					...
					if ( $oldNs && isset( $psr4[ $oldNs . '\\' ] ) ) {
						unset( $psr4[ $oldNs . '\\' ] );
						$modified = true; }
					if ( count( $srcKeys ) === 1 && ! isset( $psr4[ $newNs . '\\' ] ) ) {
						unset( $psr4[ $srcKeys[0] ] );
						$modified = true; }
		...
		if ( $oldNs && $newNs && $oldNs !== $newNs ) {
			$this->replaceInTree(
				rtrim( $base, '/\\' ) . DIRECTORY_SEPARATOR . 'src',
				array(
		...
		if ( $flags['rename-dir'] ) {
			...
			if ( $currentDir !== $newSlug ) {
				...
				} elseif ( @rename( rtrim( $base, '/\\' ), $target ) ) {
	...
	protected function parseArgs( array $args ) {
		...
		for ( $i = 0; $i < $count; $i++ ) {
			...
			if ( '' === $raw ) {
				continue; }
			if ( 0 === strpos( $raw, '--name=' ) ) {
				$out['name'] = substr( $raw, 7 );
				continue; }
			if ( '--name' === $raw && isset( $args[ $i + 1 ] ) ) {
				$out['name'] = (string) $args[ ++$i ];
				continue; }
			if ( 0 === strpos( $raw, '--slug=' ) ) {
				$out['slug'] = substr( $raw, 7 );
				continue; }
			if ( '--slug' === $raw && isset( $args[ $i + 1 ] ) ) {
				$out['slug'] = (string) $args[ ++$i ];
				continue; }
			if ( 0 === strpos( $raw, '--namespace=' ) ) {
				$out['namespace'] = substr( $raw, 12 );
				continue; }
			if ( '--namespace' === $raw && isset( $args[ $i + 1 ] ) ) {
				$out['namespace'] = (string) $args[ ++$i ];
				continue; }
			if ( 0 === strpos( $raw, '--package=' ) ) {
				$out['package'] = substr( $raw, 10 );
				continue; }
			if ( '--package' === $raw && isset( $args[ $i + 1 ] ) ) {
				$out['package'] = (string) $args[ ++$i ];
				continue; }
		}
		return $out;
	}
