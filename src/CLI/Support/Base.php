...
class Base {
	...
	protected static function parse_build_options( array $args ) {
		...
		foreach ( $args as $arg ) {
			if ( ! is_string( $arg ) || '' === $arg ) {
				continue; }
			if ( 0 === strpos( $arg, '--pm=' ) ) {
				$options['pm'] = substr( $arg, 5 ); } elseif ( 0 === strpos( $arg, '--pkgm=' ) ) {
				$options['pm'] = substr( $arg, 7 ); } elseif ( '--install' === $arg || '--force-install' === $arg ) {
					$options['force-install'] = true; } elseif ( '--no-install' === $arg ) {
					$options['skip-install'] = true; } elseif ( 0 === strpos( $arg, '--script=' ) ) {
							$script = substr( $arg, 9 );
						if ( '' !== $script ) {
	...
	protected static function install_arguments( $manager ) {
		switch ( $manager ) {
			case 'pnpm':
				return array( 'install', '--frozen-lockfile' );
			case 'yarn':
				return array( 'install', '--frozen-lockfile' );
			case 'bun':
	...
	protected static function parse_deploy_options( array $args ) {
		...
		foreach ( $args as $arg ) {
			if ( ! is_string( $arg ) || '' === $arg ) {
				continue; }
			if ( '--no-build' === $arg ) {
				$options['no-build'] = true; } elseif ( '--zip' === $arg || '--create-zip' === $arg ) {
				$options['zip'] = true; } elseif ( 0 === strpos( $arg, '--zip=' ) ) {
					$options['zip'] = true;
					$zip_value = substr( $arg, 6 );
					$options['zip-path'] = '' !== $zip_value ? $zip_value : null; } elseif ( 0 === strpos( $arg, '--pm=' ) || 0 === strpos( $arg, '--pkgm=' ) || 0 === strpos( $arg, '--script=' ) ) {
					/* handled above */ } elseif ( '--install' === $arg || '--force-install' === $arg || '--no-install' === $arg ) {
							/* handled above */ } elseif ( '-' !== substr( $arg, 0, 1 ) && null === $options['target'] ) {
							$options['target'] = $arg; }
		}
	...
	protected static function ends_with_zip( $value ) {
		return (bool) preg_match( '/\.zip$/i', (string) $value ); }
	...
	protected static function prune_vendor_tree( $vendor_path ) {
		if ( ! $vendor_path || ! is_dir( $vendor_path ) ) {
			return; }
	...
	protected static function prune_assets_tree( $assets_dir ) {
		...
		foreach ( $prune as $rule ) {
			self::delete_directory( rtrim( $assets_dir, '/\\' ) . $rule ); }
	}
	...
	protected static function create_zip_archive( $source_dir, $zip_path ) {
		if ( ! class_exists( '\\ZipArchive' ) ) {
			Console::error( 'PHP ZipArchive not available.' );
			return false; }
		$zip = new \ZipArchive();
	...
	protected static function detect_primary_namespace( $base_path ) {
		...
		if ( ! empty( $data['autoload']['psr-4'] ) && is_array( $data['autoload']['psr-4'] ) ) {
			$namespaces = array_keys( $data['autoload']['psr-4'] ); } elseif ( ! empty( $data['autoload-dev']['psr-4'] ) && is_array( $data['autoload-dev']['psr-4'] ) ) {
	...
	protected static function plugin_slug() {
		return self::sanitize_slug( basename( rtrim( self::base_path(), '/\\' ) ) ); }
	...
	protected static function search_system_path( $binary ) {
		...
		if ( 'Windows' === $platform ) {
			@exec( 'where ' . $binary . ' 2>&1', $output, $status ); } else {
			@exec( 'command -v ' . escapeshellarg( $binary ) . ' 2>&1', $output, $status ); }
	...
	protected static function resolve_wp_path( array $options ) {
		if ( isset( $options['wp-path'] ) ) {
			...
			if ( false === $resolved ) {
				Console::warning( 'Provided --wp-path could not be resolved: ' . $explicit ); } elseif ( self::looks_like_wp_root( $resolved . DIRECTORY_SEPARATOR ) ) {
				return rtrim( $resolved, '/\\' ); } else {
	...
	protected static function composer_uses_path_repositories( $composer_json ) {
		...
		foreach ( $data['repositories'] as $repository ) {
			if ( is_array( $repository ) && isset( $repository['type'] ) && 'path' === $repository['type'] ) {
				return true; }
		}
		return false;
	}
